...
class DailyWorkoutAnalysisScreen extends StatefulWidget {
	...
	@override
	State<DailyWorkoutAnalysisScreen> createState() => _DailyWorkoutAnalysisScreenState();
}

class _DailyWorkoutAnalysisScreenState extends State<DailyWorkoutAnalysisScreen> {
	bool _loading = true;
	...
	String _buildAnalysis(List<WorkoutEntry> entries) {
		if (entries.isEmpty) return 'No workout recorded for today. Log a session to get an analysis.';
		...
		buffer.writeln('Total time: $totalMinutes minutes across ${entries.length} entries.');
		buffer.writeln('Workout types: ${types.keys.join(', ')}.');

		if (totalMinutes < 20) {
			buffer.writeln('Session was short — consider adding 10–20 minutes of compound lifts or mobility.');
		} else if (totalMinutes < 45) {
			buffer.writeln('Good session length — keep progressive overload and ensure protein intake.');
		} else {
			buffer.writeln('Excellent session volume today. Watch recovery and sleep quality.');
		}

		if (types.length == 1) {
			buffer.writeln('Focus seems single-muscle — add at least one compound movement for balanced strength.');
		}

		// Lightweight actionable tip
		buffer.writeln('Tip: hydrate, log perceived exertion, and update next session targets.');
	...
	Widget build(BuildContext context) {
		return Scaffold(
			appBar: AppBar(
				title: const Text('Daily Workout Analysis'),
										...
										child: Padding(
											...
											child: Column(
												...
												children: [
													...
													Text(_todayEntries.isEmpty ? 'No workouts logged' : '${_todayEntries.length} entries'),
												],
											),
										),
									),
									...
									Card(
										child: Padding(
											...
											child: Column(
												crossAxisAlignment: CrossAxisAlignment.stretch,
												children: [
													const Text('Analysis Notes', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
													const SizedBox(height: 8),
													TextField(
														controller: _noteController,
														maxLines: 3,
														decoration: const InputDecoration(hintText: 'Add personal notes or save AI analysis here'),
													),
													const SizedBox(height: 8),
													Row(
														children: [
															Expanded(
																child: ElevatedButton(
																	...
																	await StorageService().saveAnalysisNoteForDate(_currentDate!, _noteController.text.trim());
																	ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Saved analysis note')));
																},
															...
															Expanded(
																child: OutlinedButton(
																	onPressed: () async {
																	if (_currentDate == null) return;
																	await StorageService().saveAnalysisNoteForDate(_currentDate!, _analysis);
																	_noteController.text = _analysis;
																	ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Saved AI analysis as note')));
																},
